...
#include <Geraked\\errordescription.mqh>
...
bool order(ENUM_ORDER_TYPE ot, ulong magic, double in, double sl = 0, double tp = 0, double risk = 0.01, bool martingale = false, double martingaleRisk = 0.04, int slippage = 30, bool isl = false, bool itp = false, string comment = "", string name = NULL, double vol = 0, int nRetry = 5, int mRetry = 2000, bool news = false, ENUM_NEWS_IMPORTANCE newsImportance = NEWS_IMPORTANCE_MEDIUM, int newsMinsBefore = 60, int newsMinsAfter = 60, ENUM_FILLING filling = FILLING_DEFAULT, ENUM_RISK risk_mode = RISK_DEFAULT)
{
    ...
    else if (filling == FILLING_FOK)
        req.type_filling = ORDER_FILLING_FOK;
    else if (filling == FILLING_IOK)
        req.type_filling = ORDER_FILLING_IOC;
    else if (filling == FILLING_BOC)
        req.type_filling = ORDER_FILLING_FOK;
    else if (filling == FILLING_RETURN)
        req.type_filling = ORDER_FILLING_RETURN;

    if (!OrderCheck(req, cres))
    {
        if (cres.retcode == TRADE_RETCODE_MARKET_CLOSED)
            return false;
...
bool closeOrder(ulong ticket, int slippage = 30, int nRetry = 5, int mRetry = 2000, ENUM_FILLING filling = FILLING_DEFAULT)
{
    ...
    do
    {
        ...
        if (ptype == POSITION_TYPE_BUY)
        {
            if (res.bid && Bid(psymbol) == req.price)
                req.price = res.bid;
            else
                req.price = Bid(psymbol);
        }
        ...
        {
            if (res.ask && Ask(psymbol) == req.price)
                req.price = res.ask;
            else
                req.price = Ask(psymbol);
        }
...
void checkForTrail(ulong magic, double stopLevel = 0.5, double gridStopLevel = 0.4, int slippage = 30, int nRetry = 5, int mRetry = 2000, ENUM_FILLING filling = FILLING_DEFAULT)
{
    ...
    for (int i = total - 1; i >= 0; i--)
    {
        ...
        if (pd == 0)
            continue;
        if (pstm == SYMBOL_TRADE_MODE_DISABLED || pstm == SYMBOL_TRADE_MODE_CLOSEONLY)
            continue;
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            if (StringToDouble(PositionGetString(POSITION_COMMENT)))
                k++;
        }
        ...
        if (n == 1 || k > 1)
        {
            ...
            if (!OrderSend(req, res))
            {
                if (res.retcode == TRADE_RETCODE_INVALID_STOPS && pprof - cost > 0)
                    if (closeOrder(pticket, slippage, nRetry, mRetry, filling))
                        continue;
                int err = GetLastError();
                PrintFormat("%s error #%d : %s", __FUNCTION__, err, ErrorDescription(err));
            }
...
void checkForGrid(ulong magic, double risk, double volCoef, int maxLvl, int slippage = 30, int nRetry = 5, int mRetry = 2000, bool news = false, ENUM_NEWS_IMPORTANCE newsImportance = NEWS_IMPORTANCE_MEDIUM, int newsMinsBefore = 60, int newsMinsAfter = 60, ENUM_FILLING filling = FILLING_DEFAULT)
{
    ...
    for (int i = total - 1; i >= 0; i--)
    {
        ...
        if (ptype == POSITION_TYPE_BUY)
        {
            ...
            if (!order(ORDER_TYPE_BUY, pmagic, Ask(psymbol), psl, tp, risk, false, 0, slippage, false, false, "", psymbol, vol, nRetry, mRetry, news, newsImportance, newsMinsBefore, newsMinsAfter, filling))
                continue;
        ...
        else if (ptype == POSITION_TYPE_SELL)
        {
            ...
            if (!order(ORDER_TYPE_SELL, pmagic, Bid(psymbol), psl, tp, risk, false, 0, slippage, false, false, "", psymbol, vol, nRetry, mRetry, news, newsImportance, newsMinsBefore, newsMinsAfter, filling))
                continue;
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            ZeroMemory(res);
            req.position = tickets[j];
            if (PositionGetDouble(POSITION_TP) == req.tp && PositionGetDouble(POSITION_SL) == req.sl)
                continue;
            if (!OrderSendAsync(req, res))
            {
                int err = GetLastError();
...
bool hasDealRecently(ulong magic, string symbol, int nCandles)
{
    ...
    for (int i = totalDeals - 1; i >= 0; i--)
    {
        ...
        if (HistoryDealGetString(ticket, DEAL_SYMBOL) != symbol)
            continue;
        datetime dealTime = (datetime)HistoryDealGetInteger(ticket, DEAL_TIME);
        if (TimeCurrent() < dealTime + nCandles * PeriodSeconds(PERIOD_CURRENT))
            return true;
    }
    return false;
}
...
bool fetchCalendar(int year)
{
    ...
    if (year == Tcs.year)
        date_to = Tc + PeriodSeconds(PERIOD_D1);
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (!t)
        {
            ...
            sql = "INSERT INTO country(id, name, code, currency, currency_symbol, url_name) VALUES(" + StringFormat("%d, '%s', '%s', '%s', '%s', '%s'", countries[i].id, countries[i].name, countries[i].code, countries[i].currency, countries[i].currency_symbol, countries[i].url_name) +
                  ")"
                  ";";
        ...
        for (int j = 0; j < m; j++)
        {
            ...
            if (!t)
            {
                ...
                sql = "INSERT INTO `event`(id, type, sector, frequency, time_mode, country_id, unit, importance, multiplier, digits, source_url, event_code, name) VALUES(" + StringFormat("%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, '%s', '%s', '%s'", event.id, event.type, event.sector, event.frequency, event.time_mode, event.country_id, event.unit, event.importance, event.multiplier, event.digits, event.source_url, event.event_code, event.name) +
                      ")"
                      ";";
            ...
            sql = "INSERT INTO value(id, event_id, time, period, revision, actual_value, prev_value, revised_prev_value, forecast_value, impact_type) VALUES(" + StringFormat("%d, %d, %d, %d, %d, %d, %d, %d, %d, %d", values[j].id, values[j].event_id, values[j].time, values[j].period, values[j].revision, values[j].actual_value, values[j].prev_value, values[j].revised_prev_value, values[j].forecast_value, values[j].impact_type) +
                  ")"
                  ";";
...
bool hasCurrencyNews(string currency, ENUM_NEWS_IMPORTANCE importance = 0, int minsBefore = 0, int minsAfter = 0, datetime time = 0)
{
    ...
    if (!MQLInfoInteger(MQL_TESTER))
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (!(Tc >= tb && Tc <= te))
                continue;
            if (event.importance >= (int)importance)
                return true;
        }
    ...
    if (!FileIsExist(getCalendarDbPath(), FILE_COMMON))
        return false;
