...
input group "General" input double TPCoef     = 1.0;        // TP Coefficient
...
input group "Risk Management" input double Risk                = 2.25;           // Risk
...
input group "Strategy: Grid" input bool Grid                  = true;   // Grid Enable
...
input group "News" input bool News           = false;                    // News Enable
...
void OnTimer() {
    ...
    // To Control events using the Pause Button and the dates
    if(!ui.GetTradingState() || !ui.CheckWeekdayTradingStates())
        return;
}
...
void OnTick() {
    ...
    bool couldBuy  = ui.GetBuyState();
    bool couldSell = ui.GetSellState();

    if(lastCandle != Time(0)) {
        lastCandle = Time(0);

        if(CopyBuffer(M1_handle, 0, 0, BuffSize, M1) <= 0)
            return;
        if(CopyBuffer(M2_handle, 0, 0, BuffSize, M2) <= 0)
            return;
        ArraySetAsSeries(M1, true);
        ArraySetAsSeries(M2, true);

        if(CopyBuffer(STO_handle, 0, 0, BuffSize, STO_M) <= 0)
            return;
        if(CopyBuffer(STO_handle, 1, 0, BuffSize, STO_S) <= 0)
            return;
        ...
        if(MarginLimit && PositionsTotal() > 0 && AccountInfoDouble(ACCOUNT_MARGIN_LEVEL) < MarginLimit)
            return;
        if((Grid || !MultipleOpenPos) && ea.OPTotal() > 0)
            return;

        if(BuySignal() && (Reverse ? couldSell : couldBuy))
            return;

        SellSignal() && (Reverse ? couldBuy : couldSell);
    }
}
