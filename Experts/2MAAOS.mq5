...
input group "Indicator Parameters" input int AosPeriod       = 50;            // Andean Oscillator Period
...
input group "General" input bool MultipleSymbol = true;                       // Multiple Symbols
...
input group "Risk Management" input double Risk                = 5.5;            // Risk
...
void CheckForSignal() {
    ...
    if(MarginLimit && PositionsTotal() > 0 && AccountInfoDouble(ACCOUNT_MARGIN_LEVEL) < MarginLimit)
        return;
    if(!MultipleOpenPos && ea.OPTotal() > 0)
        return;
    ...
    for(int i = 0; i < n; i++) {
        ...
        if(ea.OPTotal(s) > 0)
            continue;
        if(hasDealRecently(ea.GetMagic(), s, MinPosInterval))
            continue;
        if(SpreadLimit != -1 && Spread(s) > SpreadLimit)
            continue;
